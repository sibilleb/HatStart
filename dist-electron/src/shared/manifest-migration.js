...
exports.manifestMigration = exports.ManifestMigration = void 0;
/**
...
class ManifestMigration {
     ...
     */
    migrateCategoryManifest(manifest) {
        const changes = [];
        const warnings = [];
        const errors = [];
        try {
     ...
     */
    migrateMasterManifest(manifest) {
        const changes = [];
        const warnings = [];
        const errors = [];
        try {
            ...
            if (migrated.categoryManifests) {
                Object.keys(migrated.categoryManifests).forEach(category => {
                    const categoryManifest = migrated.categoryManifests[category];
                    const result = this.migrateCategoryManifest(categoryManifest);
                    if (result.success && result.data) {
                        migrated.categoryManifests[category] = result.data;
     ...
     */
    convertExperienceLevelToRequirement(levels) {
     ...
     */
    generateDefaultLearningResources(manifest) {
        const resources = [];
     ...
     */
    generateDifficultyIndicators(manifest) {
        const indicators = [];
        ...
        if (manifest.category === 'devops' || manifest.category === 'backend') {
            const hasGui = manifest.tags?.some(tag => tag.toLowerCase().includes('gui') ||
...
exports.ManifestMigration = ManifestMigration;
// Export singleton instance
exports.manifestMigration = new ManifestMigration();
